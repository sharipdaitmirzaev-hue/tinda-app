import type { ImageMatchStatus, IribProduct, ReviewStatus } from "./types";

export function review_product(p: IribProduct): {
  review_status: ReviewStatus;
  review_reason: string;
  confidence: IribProduct["confidence"];
} {
  ...
  if (p.duplicate_status === "exact_match" || p.duplicate_status === "probable_match") {
    reasons.push(p.duplicate_status);
  }
