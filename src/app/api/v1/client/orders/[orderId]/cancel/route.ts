...
import { cancel_client_order } from "@/lib/services/order.service";
import {
  cancel_client_order_schema,
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const input = cancel_client_order_schema.parse(body);
    const result = await cancel_client_order(payload, order_id, input);
