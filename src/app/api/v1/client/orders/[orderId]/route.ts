...
export async function GET(_request: Request, { params }: Params) {
  try {
    ...
    const result = await get_client_order(payload, order_id);
...
export async function PATCH(request: Request, { params }: Params) {
  try {
    ...
    const input = update_client_order_schema.parse(body);
    const result = await update_client_order(payload, order_id, input);
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof ZodError) {
      return api_error(
