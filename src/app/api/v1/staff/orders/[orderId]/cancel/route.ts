...
import { cancel_staff_order } from "@/lib/services/order.service";
import {
  order_id_param_schema,
  staff_cancel_order_schema,
...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const input = staff_cancel_order_schema.parse(body);
    const result = await cancel_staff_order(payload, order_id, input);
    ...
    console.error("cancel staff order error", error);
    return api_error(500, "internal_error", "Не удалось отменить заказ");
  }
}
