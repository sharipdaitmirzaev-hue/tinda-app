#!/usr/bin/env node
/**
 * Import approved_new Zelenoe ENERGY products (create-only).
 *
 *   node import-energy-products.mjs --source <json> --preview
 *   node import-energy-products.mjs --source <json> --apply \
...
const EXPECTED_CATEGORY = "energeticheskie-napitki";
const EXPECTED = 5;
...
const SOURCE =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-energy/approved-new-import-batch.json",
  );
...
const REPORT_JSON =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-energy/apply-report.json",
  );
...
function normalize_row(r, row_number) {
  ...
  return {
    ...
    category_slug: String(r.category_slug || EXPECTED_CATEGORY).trim(),
    units_per_package: 1,
...
function resolve_image_path(p) {
  ...
  const candidates = [
    path.join("/app/tmp/zelenoe-energy/original", base),
    path.join(path.dirname(SOURCE), "original", base),
    path.resolve("data/imports/zelenoe-yabloko-energy/original", base),
  ];
...
async function load_category_structure() {
  ...
  const energyish = cats.filter(
    (c) =>
      /energ|energet/i.test(c.slug) ||
      /энерг/i.test(c.name || ""),
  );
  return { all: cats, energyish };
}
...
async function main() {
  ...
  const energy_cat = cat_by_slug.get(EXPECTED_CATEGORY);
  if (!energy_cat) {
    console.error(
      "STOP: energy category missing. Do not create categories automatically.",
    );
    console.error(
      JSON.stringify(
        {
          expected: EXPECTED_CATEGORY,
          energyish_categories: category_structure.energyish,
    ...
    const stop_report = {
      ...
      reason: "energy_category_missing",
      backup_path: BACKUP,
      category_structure: category_structure.energyish,
      before,
    };
  ...
  for (const row of rows) {
    ...
    else if (String(row.brand).toLowerCase() !== "burn") {
      errors.push(`brand_not_burn:${row.brand}`);
    }
    if (!row.volume_text) errors.push("volume_text_missing");
    if (row.volume_ml !== 449) {
      errors.push(`volume_not_449:${row.volume_ml}`);
    }
    if (row.package_norm !== "can") {
      errors.push(`package_not_can:${row.package_type || row.package_norm}`);
    }
    if (!row.flavor) errors.push("flavor_missing");
    if (!row.category_slug) errors.push("category_slug_missing");
    else if (row.category_slug !== EXPECTED_CATEGORY) {
    ...
    const flavor_key = String(row.flavor || "").toLowerCase();
    if (flavor_key) {
      if (seen_flavor.has(flavor_key)) {
        errors.push(
          `flavors_merged_or_duplicate:${seen_flavor.get(flavor_key)}`,
        );
      } else {
        seen_flavor.set(flavor_key, row.row);
      }
    }
  ...
  const report = {
    ...
    note:
      "Create-only import of approved_new Zelenoe ENERGY products. No schema change. package_requires_review has no DB column — SKU list recorded below.",
    package_requires_review: true,
    ...
    category: energy_cat,
    category_structure: {
      energeticheskie_napitki: energy_cat,
      energyish: category_structure.energyish,
    },
    preview_checks: {
      candidate_count: rows.length,
      expected: EXPECTED,
      unique_proposed_sku: seen_sku.size,
      unique_flavors: seen_flavor.size,
      batch_internal_sku_dupes: rows.length - seen_sku.size,
      all_brand_burn: rows.every(
        (r) => String(r.brand || "").toLowerCase() === "burn",
      ),
      all_volume_449: rows.every((r) => r.volume_ml === 449),
      all_package_can: rows.every((r) => r.package_norm === "can"),
      preview_ok,
      preview_failed,
  ...
  const lines = [];
  lines.push("TINDA Zelenoe ENERGY products import report");
  ...
  lines.push(
    `category: ${EXPECTED_CATEGORY} (${energy_cat.name}, active=${energy_cat.is_active})`,
  );
  ...
  lines.push("");
  lines.push("SKU | status | flavor | product_id | image_url | error");
  for (const r of results) {
    lines.push(
      `${r.proposed_sku} | ${r.apply_status} | ${r.flavor} | ${r.product_id || "-"} | ${r.image_url || "-"} | ${r.error_message || "-"}`,
    );
  }
