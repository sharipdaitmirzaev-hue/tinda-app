#!/usr/bin/env node
/**
 * Second-pass import: approved_new Zelenoe juices/nectars/morses (create-only).
 *
 *   node import-juice-second-pass.mjs --source <json> --preview
 *   node import-juice-second-pass.mjs --source <json> --apply \
 *     --report-json ... --report-txt ... --backup ...
 *
 * Only creates new products. Never updates existing rows / categories.
 * Images → /uploads/products/{product_id}/{uuid}.webp
 * package_requires_review has no DB column — recorded in report only.
 *
 * Categories (existing only, do not create):
 *   kids → detskie-soki
 *   juice → sok
 *   nectar → nektar
 *   mors → mors
 *   juice_drink → sokosoderzhashchie-napitki
 * Missing category → skip with category_missing.
 */
...
function map_package_type_for_db(pkg) {
  const t = String(pkg || "").trim().toLowerCase();
  if (!t) return null;
  if (/(ж\s*\/\s*б|банка|can|жест|алюм)/.test(t)) return "банка";
  if (/(пэт|pet|пл\s*\/\s*б|п\s*\/\s*бут)/.test(t)) return "ПЭТ";
  if (/(стекл|ст\s*\/\s*б|glass)/.test(t)) return "стекло";
  if (/(тетра|tetra|пюр-пак|пюрпак)/.test(t)) return "тетрапак";
  if (/(carton|картон|пакет|pure.?pak)/.test(t)) return "картон";
  if (/(pouch|дой.?пак|дойпак)/.test(t)) return "дой-пак";
...
const SOURCE =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-juice/second-pass-import-batch.json",
  );
...
const REPORT_JSON =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-juice/second-pass-apply-report.json",
  );
...
function normalize_package(pkg) {
  ...
  if (/(carton|картон|пакет|pure.?pak)/.test(t)) return "carton";
  if (/(pouch|дой.?пак|дойпак)/.test(t)) return "pouch";
  return t || "";
}
...
function normalize_row(r, row_number) {
  ...
  const package_type = map_package_type_for_db(package_raw);
  const product_type = String(r.product_type || "").trim();
  const is_kids_line = !!r.is_kids_line;
  let category_slug = String(r.category_slug || "").trim();
  if (!category_slug) {
    if (is_kids_line) category_slug = "detskie-soki";
    else if (PRODUCT_TYPE_TO_CATEGORY[product_type]) {
      category_slug = PRODUCT_TYPE_TO_CATEGORY[product_type];
    }
  }
  const review_status = String(r.review_status || "approved_new").trim();
  const category_missing = r.category_missing === true || !category_slug;
  ...
  return {
    ...
    package_norm: normalize_package(package_raw || package_type),
...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (row.review_status !== "approved_new") {
      errors.push(`review_status_not_approved_new:${row.review_status}`);
    }
  ...
  const report = {
    ...
    note:
      "Create-only second-pass import of approved_new Zelenoe JUICE/NECTAR/MORS. No schema change. package_requires_review=true for all new SKUs (units_per_package transport packaging). Existing 314 products untouched.",
    package_requires_review: true,
    package_requires_review_detail: {
      ...
      note: "Transport packaging (units_per_package) requires clarification; units_per_package forced to 1. Applies even when package_type=картон/carton.",
    },
    category_mapping: {
      ...PRODUCT_TYPE_TO_CATEGORY,
      kids_any: "detskie-soki",
    },
    category_structure: {
      sok: cat_by_slug.get("sok") || null,
      nektar: cat_by_slug.get("nektar") || null,
      mors: cat_by_slug.get("mors") || null,
      sokosoderzhashchie_napitki:
        cat_by_slug.get("sokosoderzhashchie-napitki") || null,
      detskie_soki: cat_by_slug.get("detskie-soki") || null,
  ...
  const lines = [];
  lines.push("TINDA Zelenoe JUICE second-pass approved_new import report");
  ...
  lines.push(
    `categories present: sok=${!!cat_by_slug.get("sok")}, nektar=${!!cat_by_slug.get("nektar")}, mors=${!!cat_by_slug.get("mors")}, sokosoderzhashchie-napitki=${!!cat_by_slug.get("sokosoderzhashchie-napitki")}, detskie-soki=${!!cat_by_slug.get("detskie-soki")}`,
  );
  lines.push(
