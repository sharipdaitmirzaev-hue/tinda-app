#!/usr/bin/env node
/**
 * Import approved_new Zelenoe remaining drinks (create-only).
 *
 *   node import-remaining-drinks-products.mjs --source <json> --preview
 *   node import-remaining-drinks-products.mjs --source <json> --apply \
 ...
 *
 * Categories (existing only — do not auto-create):
 *   malt_drink            → solodovye-napitki | bezalkogolnye-napitki
 *   non_alcoholic_drink   → bezalkogolnye-napitki
 *   tonic_drink           → toniki
 *
 * If a required category is missing → stop after preview and print structure.
 */
...
const ALLOWED_CATEGORIES = new Set([
  ...
  ...Object.values(CATEGORY_FALLBACKS).flat(),
]);
...
const SOURCE =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-remaining-drinks/approved-new-import-batch.json",
  );
...
function normalize_package(pkg) {
  ...
  // Prefer glass before can — brand "Barbican" contains Latin "can"
  if (/(стекл|ст\s*\/\s*б|\bglass\b|(?:^|[\s,./(])ст(?:\.|(?=[\s,)/]|$)))/u.test(t)) {
    return "glass";
  }
  if (/(пэт|pet|пл\s*\/\s*б|пластик)/.test(t)) return "pet";
  if (/(ж\s*\/\s*б|банка|\bcan\b|жест|алюм)/.test(t)) return "can";
  return t || "";
}

function resolve_category_slug(row, cat_by_slug) {
  const candidates = [
    row.category_slug,
  ...
  for (const slug of candidates) {
    ...
    seen.add(slug);
    const cat = cat_by_slug.get(slug);
    if (cat && cat.is_active) return { slug, cat };
  }
  return { slug: row.category_slug || candidates[0] || null, cat: null };
}

function parse_volume_ml(text) {
  const t = String(text || "")
    .toLowerCase()
    .replace(",", ".");
  let m = t.match(/(\d+(?:\.\d+)?)\s*мл/);
...
function normalize_row(r, row_number) {
  ...
  const category_slug_candidates = Array.isArray(r.category_slug_candidates)
    ? r.category_slug_candidates.map((x) => String(x).trim()).filter(Boolean)
    : CATEGORY_FALLBACKS[product_type] || [];
  return {
    row: row_number,
    ...
    volume_ml:
      r.volume_ml == null ? parse_volume_ml(r.volume_text) : Number(r.volume_ml),
    package_type: package_type || null,
    package_norm: normalize_package(package_type || r.package_code || ""),
...
function resolve_image_path(p) {
  ...
  const candidates = [
    path.join("/app/tmp/zelenoe-remaining-drinks/original", base),
    path.join(path.dirname(SOURCE), "original", base),
    path.resolve(
      "data/imports/zelenoe-yabloko-remaining-drinks/original",
      base,
    ),
  ];
...
async function load_category_structure() {
  ...
  const relevant = cats.filter(
    (c) =>
      ALLOWED_CATEGORIES.has(c.slug) ||
      /napitk|tonik|solod|bezalk|gazirov|energy|chay|kvas/i.test(c.slug) ||
      /напит|тоник|солод|безалкогол|газир|энерг|чай|квас/i.test(c.name || ""),
  );
  return { all: cats, relevant };
}
...
async function main() {
  ...
  for (const row of rows) {
    const resolved = resolve_category_slug(row, cat_by_slug);
    if (!resolved.cat) {
      missing_for_types.push({
        source_name: row.source_name,
        product_type: row.product_type,
        brand: row.brand,
        tried: [
          row.category_slug,
          ...(row.category_slug_candidates || []),
        ].filter(Boolean),
      });
    } else {
      row.category_slug = resolved.slug;
      resolved_categories.set(resolved.slug, resolved.cat);
    }
  }
  ...
  for (const row of rows) {
    ...
    if (row.volume_ml == null || !Number.isFinite(row.volume_ml)) {
      errors.push("volume_missing");
    }
    if (!row.package_type) errors.push("package_missing");
    else if (!ALLOWED_PACKAGES.has(row.package_norm)) {
      errors.push(`package_undetermined:${row.package_type || row.package_norm}`);
    }
    if (!row.product_type) errors.push("product_type_missing");
    else if (!ALLOWED_PRODUCT_TYPES.has(row.product_type)) {
    ...
    if (!row.category_slug) errors.push("category_slug_missing");
      ...
      PRODUCT_TYPE_TO_CATEGORY[row.product_type] &&
      PRODUCT_TYPE_TO_CATEGORY[row.product_type] !== row.category_slug
    ) {
      errors.push(
        `category_product_type_mismatch:${row.product_type}->${row.category_slug}`,
      );
    }
    ...
    const identity = [
      ...
      .map((x) => String(x || "").toLowerCase())
      .join("|");
    if (seen_identity.has(identity)) {
      errors.push(`duplicate_identity_in_batch:${seen_identity.get(identity)}`);
    } else {
      seen_identity.set(identity, row.row);
    }
  ...
  const report = {
    ...
    categories: Object.fromEntries(
      [...resolved_categories.entries()].map(([slug, cat]) => [slug, cat]),
    ),
    category_structure: {
      relevant: category_structure.relevant,
    },
    product_type_distribution: count_by(rows, "product_type"),
    category_distribution: count_by(rows, "category_slug"),
    preview_checks: {
      candidate_count: rows.length,
      expected: EXPECTED,
      unique_proposed_sku: seen_sku.size,
      batch_internal_sku_dupes: rows.length - seen_sku.size,
      brand_defined: rows.every((r) => !!r.brand),
      flavor_defined: rows.every((r) => !!r.flavor),
      volume_defined: rows.every((r) => r.volume_ml != null),
      package_defined: rows.every((r) => !!r.package_type),
  ...
  lines.push(`product_type: ${JSON.stringify(report.product_type_distribution)}`);
  lines.push(`category_distribution: ${JSON.stringify(report.category_distribution)}`);
  ...
  lines.push("");
  lines.push("SKU | status | product_type | category | product_id | image_url | error");
