#!/usr/bin/env node
/**
 * Import approved_new Zelenoe TEA / KVASS products (create-only).
 *
 *   node import-tea-kvass-products.mjs --source <json> --preview
 *   node import-tea-kvass-products.mjs --source <json> --apply \
 ...
 *
 * Categories (existing only):
 *   iced_tea → kholodnyy-chay
 *   kvass    → kvas
 */
...
const ALLOWED_CATEGORIES = new Set(Object.values(PRODUCT_TYPE_TO_CATEGORY));
...
const SOURCE =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-tea-kvass/approved-new-import-batch.json",
  );
...
const REPORT_JSON =
  ...
  path.resolve(
    __dirname,
    "../../data/imports/zelenoe-yabloko-tea-kvass/approved-apply-report.json",
  );
...
function normalize_package(pkg) {
  ...
  if (/(пэт|pet|пл\s*\/\s*б)/.test(t)) return "pet";
...
function resolve_image_path(p) {
  ...
  const candidates = [
    path.join("/app/tmp/zelenoe-tea-kvass/original", base),
    path.join(path.dirname(SOURCE), "original", base),
    path.resolve("data/imports/zelenoe-yabloko-tea-kvass/original", base),
  ];
...
async function load_category_structure() {
  ...
  const relevant = cats.filter(
    (c) =>
      ...
      /chay|kofe|kvas|tea|kvass/i.test(c.slug) ||
      /чай|кофе|квас/i.test(c.name || ""),
  );
...
async function main() {
  ...
  const tea_cat = cat_by_slug.get("kholodnyy-chay");
  const kvass_cat = cat_by_slug.get("kvas");
  if (!tea_cat || !kvass_cat) {
    console.error(
    ...
    console.error(
      JSON.stringify(
        {
          expected: ["kholodnyy-chay", "kvas"],
          found: {
            kholodnyy_chay: !!tea_cat,
            kvas: !!kvass_cat,
          },
  ...
  for (const row of rows) {
    ...
    else if (row.package_norm !== "pet") {
      errors.push(`package_not_pet:${row.package_type || row.package_norm}`);
    }
  ...
  const report = {
    ...
    note: "Create-only import of approved_new Zelenoe TEA/KVASS. No schema change. package_requires_review has no DB column — SKU list recorded below. Existing products untouched.",
    package_requires_review: true,
  ...
  const lines = [];
  lines.push("TINDA Zelenoe TEA/KVASS approved_new import report");
  ...
  lines.push(
    `categories: kholodnyy-chay=${tea_cat.name} (active=${tea_cat.is_active}); kvas=${kvass_cat.name} (active=${kvass_cat.is_active})`,
  );
