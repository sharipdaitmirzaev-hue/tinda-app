#!/usr/bin/env tsx
/**
 * AquAlania (ООО «Константа-7») catalog importer for TINDA Market.
 *
 * Commands:
 ...
 *
 * Does NOT touch Bavaria or Daryal. Never uses --merge. Never edits existing products.
 */
...
const ARTIFACTS_ROOT = path.join(ROOT, "artifacts", "aqualania-import");
...
function run_stage1(): number {
  const script = path.join(ROOT, "scripts", "aqualania-stage1.py");
...
function cmd_discover_or_dry_run(mode: string) {
  console.log(`AquAlania ${mode}: running official-site stage1 (no DB writes)...`);
  ...
  if (!latest) {
    console.error("Stage1 finished but artifacts/aqualania-import/latest-stage1 missing");
    process.exitCode = 1;
    return;
  }
    ...
    JSON.stringify(
      {
        mode,
        artifacts: latest,
        discovered: (manifest.approved_skus?.length || 0) +
...
function load_image_buffer(row: ApprovedRow, manifest_dir: string): {
  buffer: Buffer;
  filename: string;
} | null {
...
async function cmd_apply() {
  ...
  try {
    ...
    for (const row of approved_rows) {
      ...
      try {
        ...
        const description = [
          `Производитель: ${row.manufacturer?.trim() || "ООО «Константа-7»"}`,
          `Бренд: ${row.brand?.trim() || "AquAlania"}`,
          row.flavor ? `Вкус: ${row.flavor}` : null,
          row.line ? `Линейка: ${row.line}` : null,
          row.sugar_free === "true" ? "Без сахара: да" : null,
          row.source_url ? `Источник (сайт): ${row.source_url}` : null,
          "Импорт: AquAlania official site (showcase, без цены, заказ недоступен)",
        ]
          .filter(Boolean)
          .join("\n");
        ...
        const created = await prisma.products.create({
          data: {
            ...
            brand: row.brand?.trim() || "AquAlania",
            category_id,
  ...
  const md = `# AquAlania APPLY REPORT
